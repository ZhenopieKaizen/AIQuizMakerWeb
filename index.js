...
function getPublicGeminiError(error) {
...
function splitIntoChunks(text, chunkSize = 6000, overlap = 250) {
...
function selectDocumentContext(text, intent, maxChars = MAX_CONTEXT_CHARS) {
    if (text.length <= maxChars)
        return { context: text, wasCondensed: false };
...
app.post('/api/generate-quiz', async (req, res) => {
    try {
        ...
        const { context: quizContext, wasCondensed } = selectDocumentContext(extractedText, 'Create a comprehensive quiz covering the whole document', MAX_CONTEXT_CHARS);
        const contextNotice = wasCondensed
            ...
            : '';
        const languageInstruction = config.language === 'taglish'
